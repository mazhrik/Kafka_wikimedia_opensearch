...
public class producecallback {
        private static final Logger log =LoggerFactory.getLogger(producecallback.class.getSimpleName());
...
//        creating the producer record
        for (int j=0 ; j<10;j++) {
            int i;
            for (i = 0; i < 30; i++) {
                ProducerRecord<String, String> producerRecord = new ProducerRecord<>("first_topic", "new text message" + i);
...
//                executed everytime a record is successfully sent or an exception is thrown
                        if (e == null) {
                            log.info("recieved metadata \n" + "topic" + metadata.topic() + "\n" + "patition" + metadata.partition() + "\n" + "offset" + metadata.offset() + "\n" + "timestamp" + metadata.timestamp() + "\n");
                        } else {
                            log.error("error", e);
                        }
                    }
